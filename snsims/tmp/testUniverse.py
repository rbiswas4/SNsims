...
"""
The objective of this code is generate a set of supernovae that occur in a
simulation. The set is described by a list of ntuples containing the model and
its model parameters. Reproducibility is a requirement: given a universe
instance, we want to retrieve the same supernovae independent of how the the
specfic dates and solid angle used in a query. 

The model is a description of the astronomical source, its location, underlying
source, and foregrounds (dust). SNcosmo has different sources for supernovae.
Each one can have its own rules for how they populate the universe, i.e. the
pdf of their model parameters. In addition, the realization of supernovae
depends on if the simulation is galaxy- or universe-based:

1. Given a galaxy what is the occurance of supernovae.
2. Given a universe what is the occurance of supernovae.

The generation of supernovae through galaxies is based on rates in SNU (per
galaxy) units and coordinates based relative to the galaxy. The generation of
the supernovae is based on dN/dz/dOmega (e.g. per Mpc^3) and is not tied to
galaxies. I do not see how to make generated supernovae in a single universe instantiation be identical in galaxy- and universe-based realizations. Therefore,
...
class ModelRealizer(object):
    ...
    def __init__(self, model):
        ...
        # 2 per century distributed over all self.types
        self.rate_snu = 0.05 / 365.25 / 20
        self._znorm = dict()
    ...
    def _coordsPerSlice_galaxy(self, galaxy, tindex, nsne=1):
        ...
        ans = astropy.modeling.rotations.RotateNative2Celestial.evaluate(
            phi_i, theta_i, galaxy.ra * 180 / np.pi, galaxy.dec * 180 / np.pi, 0)
    ...
    def _modelParametersPerSlice(self, host, tindex, raindex=None, nsne=1, subset='default'):
        ...
        if subset == 'default':
            ...
            calcind = np.where(subset)[0]
        ans = np.zeros(nsne)
        ans[calcind] = np.random.normal(
            size=nsne, loc=self.M0, scale=self.M0_sig)[calcind]
        return ans
    ...
    def getSNe_universe(self, universe, trange, rarange, decrange):
        ...
        for tindex in tindeces:
            for raindex in raindeces:
                ...
                if nsne != 0:
                    coords = self._coordsPerSlice_universe(
                        universe, tindex, raindex, nsne=nsne)
                    ...
                    if inrange.sum() > 0:
                        zs = self._zPerSlice_universe(
                            universe, tindex, raindex, nsne=nsne, subset=inrange)
                        mp = self._modelParametersPerSlice(
                            universe, tindex, raindex, nsne=nsne, subset=inrange)
    ...
    def getSNe_galaxy(self, galaxy, trange):
        ...
        for tindex in tindeces:
            ...
            if nsne != 0:
                ...
                if inrange.sum() > 0:
                    zs = self._zPerSlice_galaxy(
                        galaxy, tindex, nsne=nsne, subset=inrange)
                    mp = self._modelParametersPerSlice(
                        galaxy, tindex, nsne=nsne, subset=inrange)
...
for model in universe.snrules:
    ...
    zs, coords, t0, mp = models[0].getSNe_universe(
        universe, trange, rarange, decrange)
